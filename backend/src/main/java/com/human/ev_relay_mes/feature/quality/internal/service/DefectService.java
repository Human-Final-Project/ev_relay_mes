...
public class DefectService implements DefectOperations {
    ...
    public List<DefectHistoryResponseDto> search(DefectHistorySearchRequestDto condition) {
        ...
        return defectHistoryRepository.findAll(Sort.by(Sort.Direction.DESC, "occurredAt")).stream()
                ...
                .filter(item -> RequestValues.isBlank(condition.getDefectCode())
                        || item.getDefectCode().getDefectCode().equals(condition.getDefectCode()))
                .filter(item -> RequestValues.isWithinInclusive(
                        item.getOccurredAt(), condition.getStartAt(), condition.getEndAt()))
                .map(this::toResponse)
                .toList();
    }
