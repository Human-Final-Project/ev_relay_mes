...
L1 내부에서 `ERROR`가 발생해도 L1 프로세스와 TCP 연결은 종료하지 않는다. 생산 Tick만 중단하고 현재 LOT, 공정, 목표 수량, 처리 수량, Backend에 보고한 수량을 메모리에 유지한다.

오류 발생 시 L1 전송 순서:

```text
마지막 보고 이후 부분 PRODUCTION(RUNNING)
  -> ALARM(ERROR)
...
예를 들어 목표 100개 중 40개 처리 후 오류가 발생하면 `PRODUCTION inputQty=40, status=RUNNING`을 먼저 전송한다. Backend는 40개를 저장하고 남은 수량 `100 - 40 = 60`으로 `RESUME` 명령을 생성한다.
...
COMMAND_ACK(ACCEPTED)
  ...
  -> PRODUCTION(inputQty=60, COMPLETED)
  -> MACHINE_STATUS(IDLE)
```
