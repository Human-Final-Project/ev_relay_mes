...
공정 완료 시 전송한다. 설비 오류 또는 정지로 생산이 중단되면 마지막 전송 이후 처리한 부분 수량도 전송한다.
...
V1,PRODUCTION,EQ-WIND-01,OP20,EVR-LOT-20260708-002,40,40,0,RUNNING\n
```

검증 규칙:

- `INPUT_QTY = OK_QTY + NG_QTY`여야 한다.
- `STATUS`는 `RUNNING`, `COMPLETED`, `FAILED` 중 하나여야 한다.
- `LOT_NO`는 Backend에 이미 생성된 생산 LOT여야 한다.
- 정상 제품을 개별 메시지로 전송하지 않고 완료 또는 중단 구간의 수량을 요약하여 전송한다.
- `RUNNING`은 오류·정지 전에 처리했지만 아직 전송하지 않은 부분 수량에 사용한다.
- 재개 후 최종 `COMPLETED`에는 이전 부분 실적을 중복 포함하지 않고 재개 이후 남은 구간만 포함한다.
- Backend는 같은 LOT·공정의 여러 `PRODUCTION` 행을 누적하여 전체 처리 수량을 판단한다.
...
V1,COMMAND,103,RESUME,EQ-WIND-01,OP20,EVR-LOT-001,60\n
```
...
- L2는 ACK를 `POST /api/collector/command-acks`로 전달한다.
- `COMMAND_ID`는 중복 저장 방지용 이벤트 ID가 아니라 작업명령과 ACK를 연결하는 식별자다.

### 5.10 L1 설비 오류 중단·재개 순서

L1은 1초 생산 Tick마다 새로 처리된 수량을 `PRODUCTION(RUNNING)`으로 전송한다. 내부에서 `ERROR`가 발생해도 L1 프로세스와 TCP 연결은 종료하지 않고 생산 Tick만 중단하며 현재 LOT, 공정, 목표 수량, 처리 수량, Backend에 보고한 수량을 메모리에 유지한다.
...
예를 들어 목표 100개 중 40개 처리 후 오류가 발생하면 L1은 `inputQty=40`인 누적 `PRODUCTION`을 전송한다. Backend는 기존 공정 실적 행을 40개로 갱신하고 남은 수량 `100 - 40 = 60`으로 `RESUME` 명령을 생성한다.

정상 `RESUME` 수신 후 L1 전송 및 처리 순서:

```text
COMMAND_ACK(ACCEPTED)
  -> MACHINE_STATUS(RUNNING)
  -> 중단된 40개 다음부터 남은 60개 처리
  -> 1초마다 누적 PRODUCTION(RUNNING)
  -> 마지막 누적 PRODUCTION(COMPLETED)
  -> MACHINE_STATUS(IDLE)
```

다른 LOT이거나 남은 수량과 다른 `RESUME`은 `COMMAND_ACK(REJECTED)`로 거부한다. 알람 해제는 MES 사용자가 처리하므로 L1은 알람 해제 메시지를 보내지 않는다.
...
REST API 경로는 `/api/collector` 아래에서 이벤트별로 분리한다.
...
| `PRODUCTION`     | 예           | 정상 완료 또는 오류·정지 시 구간 생산실적을 `production_logs`에 저장 |
...
| 용도              | API 경로                              |
| ----------------- | ------------------------------------- |
| 대기 명령 Polling | `GET /api/collector/commands/pending` |
| 명령 ACK          | `POST /api/collector/command-acks`    |
...
- 현재 Backend `ProductionResultReceiveRequestDto`는 생산 상태로 `RUNNING`, `COMPLETED`를 허용하므로 부분 실적과 완료 실적은 그대로 전달한다. TCP 명세의 `FAILED`를 실제로 사용하려면 Backend 상태 계약을 먼저 수정해야 한다. 계약 수정 전 `FAILED`를 전송하면 HTTP 400으로 처리한다.
...
- [x] 생산 상태 `RUNNING`, `COMPLETED`, `FAILED` 적용
- [x] 검사 결과를 SQL 기준 `OK`, `NG`로 적용
- [x] OP20·OP30 병렬 실행 및 OP40_OP50 합류 구조 적용
- [x] 일반 공정은 1초마다 누적 `PRODUCTION(RUNNING)`, 마지막 Tick에 `COMPLETED` 전송
