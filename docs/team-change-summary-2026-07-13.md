...
- 공정 완료 시 LOT 단위 생산 요약을 `production_logs`에 저장한다.
...
| 데이터 | 저장 기준 |
|---|---|
| `PRODUCTION` | 공정 완료 시 항상, LOT·공정별 생산 요약 |
...
- 공정 완료 시 `PRODUCTION` 요약을 반드시 전송한다.
...
- 공정별 `production_logs` 요약과 불량·알람·상태 변경을 저장한다.
