...
function lotAutomationLabel(lot){
  ...
  if(lot.status==="RUNNING") return lot.lotType==="SUPPLEMENT" ? "자동 보충 생산 중" : "자동 생산 중";
  if(lot.status==="HOLD") return "설비 복구 대기";
  if(lot.status==="COMPLETED") return "완료";
  if(lot.status==="SCRAPPED") return "폐기";
  return "-";
}
